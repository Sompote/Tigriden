...
![Tigriden with paperIEE.tex typeset in the viewer — a two-column journal page with figures and captions — the manuscript folder in the file panel on the left, and Claude Code running in the terminal below](assets/screenshot.png)
...
terminal take their own font and their own size; the file is
...
Both families are checked against the font database at startup: one this machine
does not have is replaced by the first installed fallback for the platform
(Menlo on macOS, Cascadia Mono on Windows, DejaVu Sans Mono elsewhere) and
written back, so what the file says and what you see agree. A config from before
0.1.8 has no `term_font_family` and keeps one family for both until you pick a
terminal font.
...
The frame cap is a ceiling, not a target. The terminal paints when a program
writes to it, and the cap only decides how many of those writes get a paint of
their own, so it costs nothing while a program writes more slowly than the cap.
Over 12 s of streaming output, 60 fps cost 3.48 s of CPU, 30 fps cost 3.30 s and
15 fps cost 1.83 s. That is one run per setting with no repeats, so the ordering
is reliable and the individual figures are not. The lean value is 15 rather than
30 because 30 was measured to buy almost nothing. A cap only saves work once it
sits below the rate at which the program damages the grid, and that rate was near
30/s here, so the 60 fps cap never bound and the 30 fps cap barely did. Settings
offers 15, 30 and 60; `config.toml` takes anything from 10 to 120.
...
already open without reopening them. Turning the watch off stops anything
refreshing by itself: the file tree and the Changes panel update when you next
act on them, and an open document stops re-rendering when it changes on disk.
...
- **0.2.1** — **What a window costs while an agent streams into it.**
  ...
  **Four settings, one switch.** The frame cap is a setting, because the right value depends on how many windows are streaming at once. Settings ▸ Terminal offers 15, 30 and 60 fps, and `config.toml` takes 10 to 120 as `term_fps`. Over 12 s of streaming output, 60 fps cost 3.48 s of CPU, 30 fps cost 3.30 s and 15 fps cost 1.83 s, one run per setting with no repeats, so the ordering is reliable and the individual figures are not. Dropping to 30 buys almost nothing, because a cap only saves work once it sits below the rate at which the program damages the grid. That rate was near 30/s here, so the 60 fps cap never bound and the 30 fps cap barely did. **Watch files** is the second new setting. Off stops the recursive watch on open folders, so a build or a training run that churns thousands of files no longer wakes the window. Nothing then refreshes by itself: the file tree and the Changes panel update when you next act on them, and an open document stops re-rendering when it changes on disk. The setting applies to folders already open, without reopening them. **Lean mode** sets the frame cap to 15, the scrollback to 2000, the Changes panel off and file watching off, for the case of several windows running agents side by side. It is derived from those four values rather than stored, so it cannot disagree with them: it reads On only while all four are lean, and goes out as soon as one is moved back on its own. Turning it off restores the defaults rather than the values it replaced.
